...
function detectLoans(carriera) {
  // Per ogni stint con apps>0, verifica se ESISTE UN'ALTRA ENTRY (NON mergiata)
  // di un altro club il cui range CONTIENE STRETTAMENTE l'attuale stint
  // (entrambi i confini strettamente esterni). Evita falsi positivi quando
  // un club ha più stint disgiunti che merging-li darebbe un range troppo largo.
  return carriera.map(c => {
    ...
    const isLoan = carriera.some(other => {
      ...
      if (!os) return false;
      // Entrambi i confini strettamente esterni
      return os < cs && oe > ce;
    });
